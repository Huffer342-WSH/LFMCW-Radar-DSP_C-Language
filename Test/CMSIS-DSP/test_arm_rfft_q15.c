...
#include "./data/fft_data_64_complex.c"
    ...
    double rmse = 0;
    printf("%3s %6s %6s %s  %s %s %s\n", "i", "Q15-Real", "Q16-Imag", "Double-Real", "Double-Imag",
           "倍数-Real", "倍数-Imag");
    for (int i = 0; i < FFT_LEN; i++) {
        printf("%4i %+6hd  %+6hd   %+.3e   %+.3e     %3.1f      %3.1f\n", //
               i,                                                         //
        ...
        rmse += powl(FFT_OUT[i * 2] / FFT_LEN - outq15[i * 2], 2) +
                powl(FFT_OUT[i * 2 + 1] / FFT_LEN - outq15[i * 2 + 1], 2);
    }
    printf("均方根误差RMSE = %f\n", rmse / FFT_LEN);
    printf("\n");
